...
void node_basic_multi_test(void)
{
    ...
    DeclareVar aDecl(&a, &one_plus_two);
    ...
    DeclareVar bDecl(&b, &four_plus_five);
    aDecl.setNext(&bDecl);
    ...
    DeclareVar cDecl(&c, &ten_plus_eleven);
    bDecl.setNext(&cDecl);

    std::ostringstream ss;
    ss << "DeclareVar=[ident=[Ident[a]] expr=[Tuple=[lhs=[IntLiteral=[1]] op=[+] rhs=[IntLiteral=[2]]]]]\n";
    ss << "DeclareVar=[ident=[Ident[b]] expr=[Tuple=[lhs=[IntLiteral=[4]] op=[+] rhs=[IntLiteral=[5]]]]]\n";
    ss << "DeclareVar=[ident=[Ident[c]] expr=[Tuple=[lhs=[IntLiteral=[10]] op=[+] rhs=[IntLiteral=[11]]]]]\n";
