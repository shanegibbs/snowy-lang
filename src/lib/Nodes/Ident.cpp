...
Ident::Ident(const shared_ptr<const string> s, const TypePtr t)
    : name(s), type(t) {
  s_assert(type);
  log.debug("Creating Identity '%s' with type '%s'", name->c_str(),
            type->getId().c_str());
  init();
}
...
const string *Ident::getName() const { return name.get(); }

const TypePtr Ident::getType() const { return type; }
...
Value *Ident::compile(CodeGen &gen) const {
  ...
  const Value *ptr_to_load =
      gen.getValue(name_string);  // needs to come from somewhere
