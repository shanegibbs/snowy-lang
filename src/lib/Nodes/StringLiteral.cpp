...
Value *StringLiteral::compile(CodeGen &gen) const {
  ...
  // global value
  ArrayType *gv_arr_ty =
      ArrayType::get(llvm::Type::getInt8Ty(*context), actual.length() + 1);
  ...
  GlobalVariable *str_lit =
      new GlobalVariable(*gen.getModule(), gv_arr_ty, true,
                         GlobalValue::ExternalLinkage, str_init, "str_lit");

  // pointer to global value
  Constant *eptr_args[2] = {ConstantInt::get(*context, APInt(8, 0, false)),
